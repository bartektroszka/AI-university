...
def make_move(move, end_points, board, blacks, whites):
    x, y = move
...
def alphabeta(board, depth, a, b, maximizingPlayer, turn, passes, free, blacks, whites):
    ...
    if maximizingPlayer:
        ...
        pos_moves = legal_moves(board, turn)
        if not pos_moves:
            value = max(value, alphabeta(board, depth -1, a, b, False, turn%2 + 1, passes+1, free, blacks, whites))
        else:
            for move, endpoints in pos_moves.items():
                child, blacks, whites = make_move(move, endpoints, copy.deepcopy(board), blacks, whites)
                value = max(value, alphabeta(child, depth -1, a, b, False, turn%2 + 1, 0, free - 1, blacks, whites))
                a = max(a, value)
                if a >= b:
                    break
        return value
        ...
        pos_moves = legal_moves(board, turn)
        if not pos_moves:
            value = min(value, alphabeta(board, depth -1, a, b, True, turn%2 + 1, passes+1, free, blacks, whites))
        else:
            for move, endpoints in pos_moves.items():
                child, blacks, whites = make_move(move, endpoints, copy.deepcopy(board), blacks, whites)
                value = min(value, alphabeta(child, depth -1, a, b, True, turn%2 + 1, 0, free - 1, blacks, whites))
                b = min(b, value)
                if a >= b:
                    break
        return value
...
def smart_agent(board, passes, free, turn, blacks, whites):
    pos_moves = legal_moves(board, turn)
    ...
    if not pos_moves:
        ...
        for move, endpoints in  pos_moves.items():
            child, blacks, whites = make_move(move, endpoints, copy.deepcopy(board), blacks, whites)
            value =  alphabeta(child, 2, float("-inf"), float("inf"), True, turn, passes, free, blacks, whites)
...
for i in range(100):
    ...
    while free:
        ...
        move = 0
        if turn == 1:
            move = smart_agent(board, passes, free, turn,blacks, whites)
        ...
        if move == 0:
            ...
            board, black, whites = make_move(move[0], move[1], board, blacks, whites)
            free -= 1
