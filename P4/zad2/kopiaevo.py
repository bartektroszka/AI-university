...
def random_heuristics(n):
    ...
    for i in range(n):
        ...
        dist_to_enemy_cave = random.randint(0,40)
        own_traps_penalty = random.randint(-100, 0)
        win_bonus = random.randint(100,1000)
        lose_penalty = random.randint(-1000, -100)
        list_of_heur.append([r_value, c_value, d_value, w_value, j_value, t_value, l_value, e_value, dist_to_enemy_cave,  own_traps_penalty, win_bonus, lose_penalty])
    return list_of_heur
        ...
        for j in range(7):
            ...
            if figure in ["R", "C", "D", "W", "J", "T", "L", "E"]:
                if board[i][j] == "*" and i>3:
                    summ = heur[10]
                    return summ
                summ -= ((8 - i) + abs(3 - j)) * heur[8]
                if board[i][j] == "#" and i < 5:
                    summ += heur[9]
                summ += heur[["R", "C", "D", "W", "J", "T", "L", "E"].index(figure)]
...
def cross(heur_1, heur_2):
    r_value = random.choice([heur_1[0], heur_2[0]])
    c_value = random.choice([heur_1[1], heur_2[1]])
    d_value = random.choice([heur_1[2], heur_2[2]])
    w_value = random.choice([heur_1[3], heur_2[3]])
    j_value = random.choice([heur_1[4], heur_2[4]])
    t_value = random.choice([heur_1[5], heur_2[5]])
    l_value = random.choice([heur_1[6], heur_2[6]])
    e_value = random.choice([heur_1[7], heur_2[7]])
    dist_to_enemy_cave = random.choice([heur_1[8], heur_2[8]])
    own_traps_penalty = random.choice([heur_1[9], heur_2[9]])
    win_bonus = random.choice([heur_1[10], heur_2[10]])
    lose_penalty = random.choice([heur_1[11], heur_2[11]])
    return [r_value, c_value, d_value, w_value, j_value, t_value, l_value, e_value, dist_to_enemy_cave,  own_traps_penalty, win_bonus, lose_penalty]

def cross_generation(heurs, final_pop, mutation_factor):
    result = heurs
    for i in range(final_pop - len(heurs)):
...
def mutation(heur, mutation_factor):
    for i in range(len(heur)):
        heur[i] = heur[i] * random.gauss(1, mutation_factor)
    return heur

def selection(heurs, final_number, number_of_fights):
    heurs_points = []
    for heur in heurs:
        heurs_points.append([heur, 0])
        points = 0
        for i in range(number_of_fights):
            enemy = random.choice(heurs)
            if play_game(heur, enemy, deployment, values) == 1:
                points += 1
            if play_game(enemy, heur, deployment, values) == 2:
                points += 1
        heurs_points.append((heur, points))
    heurs_points.sort(key=lambda elem: elem[1])
    result = []
    for i in range(final_number):
        result.append(heurs_points[-i-1][0])
    return result
...
for j in range(1):
    ...
    heur = random.choice(evolution(100, 0.15, 30, 10, 100))
    print(heur)
    heur_score = 0
    for i in range(1000):
        values = {"R":1 ,"C":2, "D":3, "W":4, "J":5, "T":6, "L":7, "E":8, "r":1 ,"c":2, "d":3, "w":4, "j":5, "t":6, "l":7, "e":8, "N":0, "n":0}
        ...
        deployment = [["L",".",".",".",".",".","T"],
                        ...
                        [".","d",".",".",".","c","."],
                        ["t",".",".",".",".",".","l"]]
        if play_game_vs_random(heur, deployment, values) == 1:
            heur_score += 1
    print(heur_score)
