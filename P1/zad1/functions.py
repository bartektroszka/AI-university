        ...
        for i in range(3):
            for j in range(3):
                ...
                (not check_for_black(new_pos, black_king_pos, rook_pos))):
                    possibilities.append([size, new_pos, black_king_pos, rook_pos, 0])
        ...
        for i in range(3):
            for j in range(3):
                ...
                (not check_for_white(white_king_pos, new_pos, rook_pos))):
                    possibilities.append([size, white_king_pos, new_pos, rook_pos, 1])
        return possibilities
    ...
    def rook_pos_moves():  
        ...
        for i in range(size):
            if not ((rook_pos[0] <= white_king_pos[0] <= i) and (rook_pos[1] == white_king_pos[1])):
                if i != rook_pos[0]:
                    possibilities.append([size, white_king_pos, black_king_pos, [i, rook_pos[1]], 0])
            if not ((rook_pos[1] <= white_king_pos[1] <= i) and (rook_pos[0] == white_king_pos[0])):
                if i != rook_pos[1]:
                    possibilities.append([size, white_king_pos, black_king_pos, [rook_pos[0], i], 0])
        return possibilities
